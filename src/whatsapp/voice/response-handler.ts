...
export class WhatsAppVoiceResponseHandler {
  ...
  async sendVoiceResponse(
    options: WhatsAppVoiceResponseOptions,
  ): Promise<WhatsAppVoiceUploadResult | string> {
  ...
  private async uploadVoiceFile(
    ...
    phoneNumber: string,
    _text: string,
  ): Promise<WhatsAppVoiceUploadResult> {
    try {
      // Read file
      ...
      if (!messageResponse.ok) {
        throw new Error(
          `Message sending failed: ${messageResponse.status} ${messageResponse.statusText}`,
        );
      }
  ...
  async cleanupOldFiles(maxAgeMs: number = 3600000): Promise<void> {
    try {
      ...
      this.runtime.error?.(`Failed to cleanup old files: ${errMsg}`);
    }
  }
}
