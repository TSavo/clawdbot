...
import { handleWhatsAppVoiceMessage, shouldHandleWhatsAppVoiceMessage } from "../integration.js";
...
describe("WhatsApp Voice: Message Handler", () => {
  ...
  it("should identify audio files correctly", () => {
    expect(handler.isAudioFile({ id: "test", mimetype: "audio/ogg", timestamp: Date.now() })).toBe(
      true,
    );
    expect(handler.isAudioFile({ id: "test", mimetype: "audio/mpeg", timestamp: Date.now() })).toBe(
      true,
    );
    expect(handler.isAudioFile({ id: "test", mimetype: "audio/mp3", timestamp: Date.now() })).toBe(
      true,
    );
    expect(handler.isAudioFile({ id: "test", mimetype: "image/jpeg", timestamp: Date.now() })).toBe(
      false,
    );
  });
});
