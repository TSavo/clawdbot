...
export async function handleWhatsAppVoiceMessage(params: {
  ...
  const { message, apiClient, runtime: _runtime, providersConfig, replyFn, sendFn } = params;
  ...
  try {
    ...
    logVerbose(`whatsapp-voice: downloaded audio (size: ${audioBuffer.length} bytes)`);
 ...
 */
async function downloadWhatsAppAudio(apiClient: any, mediaId: string): Promise<Buffer> {
