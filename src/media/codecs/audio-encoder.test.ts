...
describe("AudioEncoder", () => {
  ...
  describe("MP3 Encoding", () => {
    ...
    it("should throw on invalid PCM buffer", async () => {
      await expect(encoder.encodeToMP3(Buffer.alloc(0), 16000, 64)).rejects.toThrow(
        AudioEncodingError,
      );
    });
    ...
    it("should throw on invalid sample rate", async () => {
      ...
      await expect(encoder.encodeToMP3(pcmBuffer, 1000, 64)).rejects.toThrow("Invalid sample rate");

      await expect(encoder.encodeToMP3(pcmBuffer, 100000, 64)).rejects.toThrow(
        "Invalid sample rate",
      );
    });
    ...
    it("should throw on invalid bitrate", async () => {
      ...
      await expect(encoder.encodeToMP3(pcmBuffer, 16000, 10)).rejects.toThrow("Invalid bitrate");

      await expect(encoder.encodeToMP3(pcmBuffer, 16000, 200)).rejects.toThrow("Invalid bitrate");
    });
  ...
  describe("OGG Encoding", () => {
    ...
    it("should throw on invalid PCM buffer", async () => {
      await expect(encoder.encodeToOGG(Buffer.alloc(0), 16000, 64)).rejects.toThrow(
        AudioEncodingError,
      );
    });
    ...
    it("should throw on invalid sample rate", async () => {
      ...
      await expect(encoder.encodeToOGG(pcmBuffer, 1000, 64)).rejects.toThrow("Invalid sample rate");
    });
...
describe("Integration with Discord Response Handler", () => {
  ...
  it("should handle typical Discord voice message sizes", async () => {
    ...
    for (const { duration, description: _description } of testCases) {
