...
export class OpusCodec extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.decoder = new OpusEncoder(this.config.sampleRate, this.config.channels);
  ...
  encode(pcm: Int16Array | Buffer): Buffer {
    ...
    try {
      ...
      const actualSamples = pcm instanceof Int16Array ? pcm.length : pcm.length / 2; // 16-bit = 2 bytes per sample
