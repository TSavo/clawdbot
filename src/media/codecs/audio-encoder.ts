...
class MP3Encoder {
   ...
   */
  async encode(pcmBuffer: Buffer, options: Required<EncoderOptions>): Promise<Buffer> {
    await this.initialize();

    try {
      const { sampleRate, bitrate, channels } = options;

      logger.warn("Using fallback PCM wrapper for MP3. Consider using OGG format for production.");
...
class OGGEncoder {
   ...
   */
  private wrapInOggContainer(opusData: Buffer, sampleRate: number, channels: number): Buffer {
...
export class AudioEncoder {
   ...
   */
  estimateEncodingTime(pcmSize: number, format: "mp3" | "ogg"): number {
