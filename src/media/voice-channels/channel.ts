...
export class VoiceChannel extends EventEmitter {
   ...
   */
  async addParticipant(config: VoiceParticipantConfig): Promise<VoiceParticipant> {
    if (this.config.maxParticipants && this.participants.size >= this.config.maxParticipants) {
      throw new Error(
    ...
    for (const [, participant] of this.participants) {
      if (participant.isConnectedToChannel()) {
        try {
          ...
          console.warn(`Failed to send audio to participant ${participant.id}:`, error);
        }
      }
    }
  }
