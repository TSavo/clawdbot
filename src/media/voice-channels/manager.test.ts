...
class ChannelManager {
   ...
   */
  async joinChannel(channelId: string, participantConfig: VoiceParticipantConfig): Promise<void> {
...
describe("Channel Manager", () => {
  ...
  describe("Channel Lifecycle", () => {
    ...
    it("should not allow duplicate channel IDs", async () => {
      ...
      await expect(manager.createChannel(config)).rejects.toThrow("already exists");
    });
  ...
  describe("Error Handling", () => {
    it("should handle joining non-existent channel", async () => {
      await expect(manager.joinChannel("non-existent", { userId: "user-1" })).rejects.toThrow(
        "not found",
      );
    });

    it("should handle leaving non-existent channel", async () => {
      await expect(manager.leaveChannel("non-existent", "user-1")).rejects.toThrow("not found");
    });
