...
export class AudioMixer {
  ...
  getActiveTracks(): string[] {
    return Array.from(this.tracks.values())
      .filter((t) => t.enabled && t.lastEnergy > this.config.vadThreshold)
      .map((t) => t.userId);
  }
  ...
  private mixBroadcast(samples: Map<string, Float32Array>): Float32Array {
    const maxLength = Math.max(...Array.from(samples.values()).map((s) => s.length));
  ...
  private mixSelective(samples: Map<string, Float32Array>): Float32Array {
    ...
    // Take top 4 speakers
    const topN = 4;
    const selected = new Map(sorted.slice(0, topN));
  ...
  private collectSamples(): Map<string, Float32Array> {
    ...
    for (const [userId, track] of this.tracks) {
      ...
      const combined = this.concatenateAudioBuffers(track.buffers);
   ...
   */
  private concatenateAudioBuffers(buffers: AudioBuffer[]): Float32Array {
    const totalSamples = buffers.reduce((sum, buf) => sum + buf.data.length / 2, 0);
   ...
   */
  private float32ToAudioBuffer(float32: Float32Array): AudioBuffer {
