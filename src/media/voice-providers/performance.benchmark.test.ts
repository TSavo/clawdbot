...
describe("Performance Benchmarks", () => {
  describe("Streaming Latency", () => {
    it("should benchmark Cartesia first chunk latency (<100ms)", async () => {
      ...
      expect(stats.p95LatencyMs).toBeLessThan(
        PERFORMANCE_TARGETS.streaming.cartesiaFirstChunk * 1.5,
      );
  ...
  describe("Broadcasting Performance", () => {
    it("should broadcast to 16 participants in <50ms", () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        void Promise.all(broadcasts).then(() => {
          const latency = performance.now() - startTime;
          metrics.recordLatency(latency);
        });
      }
  ...
  describe("System Integration Performance", () => {
    it("should maintain <150ms end-to-end latency", () => {
      ...
      for (let i = 0; i < 100; i++) {
        ...
        const totalLatency =
          transcriptionLatency + processingLatency + synthesisLatency + outputLatency;
    ...
    it("should verify no degradation over 1 hour (simulated)", async () => {
      ...
      const firstHalfAvg =
        metrics["latencies"].slice(0, operations / 2).reduce((a, b) => a + b, 0) / (operations / 2);

      const secondHalfAvg =
        metrics["latencies"].slice(operations / 2).reduce((a, b) => a + b, 0) / (operations / 2);
