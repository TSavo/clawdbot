...
import { AudioFormat as AF, BaseVoiceProviderExecutor, VoiceProviderError } from "./executor.js";
...
class WhisperDockerDeployment {
  ...
  async start(): Promise<void> {
    try {
      ...
      throw new Error(
        `Failed to start Whisper container: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      console.warn(
        `Failed to stop Whisper container: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
...
class WhisperSystemDeployment {
  ...
  constructor(modelSize: string = "base", pythonPath?: string, cachePath?: string) {
    this.modelSize = modelSize;
    this.pythonPath = pythonPath || "python3";
    this.cachePath =
      cachePath || process.env.HOME ? `${process.env.HOME}/.cache/whisper` : "/tmp/whisper-cache";
  }
...
export class WhisperExecutor extends BaseVoiceProviderExecutor {
  ...
  constructor(
    providerId: string,
    config: z.infer<
      typeof import("../../config/zod-schema.voice-providers.js").WhisperConfigSchema
    >,
  ) {
    super();
    this.id = providerId;
  ...
  async transcribe(audio: AudioBuffer, options?: TranscribeOptions): Promise<TranscriptionResult> {
    if (!this.initialized) {
      throw new VoiceProviderError("Executor not initialized", this.id, "NOT_INITIALIZED");
    }

    // Normalize audio format
    const normalized = this.normalizeAudioBuffer(audio, AF.PCM_16, 16000);
    ...
    throw new VoiceProviderError("No deployment handler available", this.id, "NO_HANDLER");
  }
  ...
  async *transcribeStream(
    ...
    try {
      ...
      while (true) {
        ...
        const normalized = this.normalizeAudioBuffer(value, AF.PCM_16, 16000);
   ...
   */
  async *synthesizeStream(_textStream: ReadableStream<string>): AsyncIterable<never> {
  ...
  getCapabilities(): ProviderCapabilities {
    return {
      ...
      maxConcurrentSessions: this.config.deploymentMode === "docker" ? 4 : 1,
      estimatedLatencyMs: 2000,
      requiresNetworkConnection: false,
      requiresLocalModel:
        this.config.deploymentMode === "docker" || this.config.deploymentMode === "system",
    };
  }
