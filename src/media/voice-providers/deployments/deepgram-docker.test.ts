...
describe("DeepgramDockerHandler", () => {
  ...
  describe("WebSocket Connection Management", () => {
    it("should fail to create connection without assigned port", async () => {
      await expect(handler.createWebSocketConnection()).rejects.toThrow(VoiceProviderError);
    });
  ...
  describe("Health Checking", () => {
    ...
    it("should timeout after max wait duration", async () => {
      await expect(handler.waitForHealthy("http://127.0.0.1:9999/health", 100)).rejects.toThrow(
        VoiceProviderError,
      );
    });
  });
