...
describe("DockerHandler", () => {
  ...
  beforeEach(async () => {
    ...
    vi.mocked(promisify).mockImplementation((_fn: any) => {
  ...
  afterEach(async () => {
    try {
      if (handler?.getRunningContainerId?.()) {
        ...
        mockExecSync.mockImplementation((_cmd: string, cb: any) => {
  ...
  describe("pullImage", () => {
    ...
    it("should throw after max retries exhausted", async () => {
      ...
      await expect(handler.pullImage("kokoro:latest", 2)).rejects.toThrow(VoiceProviderError);
    }, 15000);
  });
  ...
  describe("startContainer", () => {
    ...
    it("should throw if config is missing", async () => {
      await expect(handler.startContainer(undefined as any)).rejects.toThrow(VoiceProviderError);
    });
  });
  ...
  describe("stopContainer", () => {
    it("should stop container gracefully", async () => {
      ...
      await expect(handler.stopContainer("container-123")).resolves.not.toThrow();
    });
  ...
  describe("waitForHealthy", () => {
    it("should timeout on failed health checks", async () => {
      vi.mocked(http.get).mockImplementation((_options: any, _callback: any) => {
        return {
          on: (event: string, handler: Function) => {
            if (event === "error") {
      ...
      await expect(handler.waitForHealthy("http://localhost:8000/health", 100)).rejects.toThrow(
        VoiceProviderError,
      );
    });
  ...
  describe("synthesizeStream", () => {
    ...
    it("should throw error on synthesis failure", async () => {
      ...
      await expect(handler.synthesizeStream("Test text")).rejects.toThrow(VoiceProviderError);
    });
