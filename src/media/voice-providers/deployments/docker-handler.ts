  ...
  async waitForHealthy(endpoint: string, maxWaitMs: number = 30000): Promise<void> {
    ...
    while (Date.now() - startTime < maxWaitMs) {
      ...
      try {
        ...
        console.log(
          `[Docker] Health check attempt ${attemptCount} failed: ${errorMsg} (${elapsed}ms elapsed)`,
        );
  ...
  private checkHealthEndpoint(endpoint: string): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        request.on("socket", (_socket) => {
          console.log(`[Docker] Health check socket connected to ${hostname}:${port}`);
        });
      } catch (error) {
        console.log(
          `[Docker] Health check exception: ${error instanceof Error ? error.message : String(error)}`,
        );
        reject(error);
      }
    });
