...
export class SystemHandler {
    ...
    async detectPython() {
        ...
        for (const candidate of pythonCandidates) {
            try {
                ...
                if (versionMatch) {
                    ...
                    console.warn(`[System] ${candidate} version ${major}.${minor} is below minimum 3.9`);
                }
            }
            catch (error) {
        ...
        throw new VoiceProviderError('Python 3.9+ not found. Please install Python 3.9 or later.', 'kokoro-system', 'PYTHON_NOT_FOUND');
    }
    /**
     * Check if kokoro is installed via pip
     */
    async checkKokoroInstalled(pythonPath) {
        try {
     ...
     */
    async installKokoro(pythonPath, installCmd) {
        const cmd = installCmd || `${pythonPath} -m pip install kokoro`;
        try {
            ...
            if (stdout)
                console.log('[System] Install stdout:', stdout);
            if (stderr)
                console.log('[System] Install stderr:', stderr);
            console.log('[System] Kokoro installed successfully');
        }
        catch (error) {
            throw new VoiceProviderError(`Failed to install Kokoro: ${error instanceof Error ? error.message : String(error)}`, 'kokoro-system', 'INSTALL_FAILED');
        }
    }
    /**
    ...
    async startProcess(config) {
        try {
            ...
            throw new VoiceProviderError(`Failed to start Kokoro process: ${error instanceof Error ? error.message : String(error)}`, 'kokoro-system', 'PROCESS_SPAWN_FAILED');
        }
    }
    /**
     ...
     */
    async stopProcess(pid, timeoutMs = 10000) {
        try {
            ...
            throw new VoiceProviderError(`Failed to stop process ${pid}: ${error instanceof Error ? error.message : String(error)}`, 'kokoro-system', 'PROCESS_SHUTDOWN_FAILED');
        }
    }
    /**
     * Get process status
    ...
    async getProcessStatus(pid) {
        try {
            ...
            try {
                const { stdout } = await execAsync(`ps -p ${pid} -o etime=,rss= 2>/dev/null || tasklist /FI "PID eq ${pid}" 2>/dev/null`);
                // Parse output
