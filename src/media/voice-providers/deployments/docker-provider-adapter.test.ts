...
describe("DockerProviderAdapter", () => {
  ...
  it("should create provider instance with default config", async () => {
    const handler = await adapter.createProviderInstance("test-whisper", "faster-whisper");
  ...
  it("should throw for unsupported provider in createProviderInstance", async () => {
    await expect(adapter.createProviderInstance("test", "unsupported-provider")).rejects.toThrow(
      VoiceProviderError,
    );
  });
