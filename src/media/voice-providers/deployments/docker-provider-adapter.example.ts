...
export async function exampleBasicDeployment(): Promise<void> {
  ...
  const handler = await adapter.createProviderInstance("whisper-default", "faster-whisper");
...
export async function exampleGpuAcceleration(): Promise<void> {
  ...
  await adapter.createProviderInstance("whisper-gpu", "faster-whisper", config);
...
export async function exampleMultipleSttInstances(): Promise<void> {
  ...
  await adapter.createProviderInstance("whisper-fast", "faster-whisper", {
    port: 8001,
    env: {
  ...
  await adapter.createProviderInstance("whisper-accurate", "faster-whisper", {
    port: 8002,
    gpuEnabled: true,
    memoryLimit: "12g",
    env: {
...
export async function exampleChatterboxTts(): Promise<void> {
  ...
  await adapter.createProviderInstance("chatterbox-main", "chatterbox", config);
...
export async function exampleVoicePipeline(): Promise<void> {
  ...
  await adapter.createProviderInstance("pipeline-stt", "faster-whisper", {
    port: 8001,
    gpuEnabled: true,
    env: {
      DEFAULT_MODEL_SIZE: "base",
  ...
  await adapter.createProviderInstance("pipeline-tts", "chatterbox", {
    port: 5000,
    gpuEnabled: true,
    env: {
      TTS_ENGINE: "glow-tts",
...
export async function examplePortAllocation(): Promise<void> {
  ...
  const handler1 = await adapter.createProviderInstance("instance-1", "faster-whisper");

  const handler2 = await adapter.createProviderInstance("instance-2", "faster-whisper");

  const handler3 = await adapter.createProviderInstance("instance-3", "faster-whisper");
...
export async function exampleTemplateUsage(): Promise<void> {
  ...
  await adapter.createProviderInstance("whisper-from-template", "faster-whisper", {
...
export async function exampleErrorHandling(): Promise<void> {
  ...
  try {
    ...
    await adapter.createProviderInstance("unknown-instance", "nonexistent-provider");
  } catch (error) {
...
export async function exampleProductionSetup(): Promise<void> {
  ...
  try {
    ...
    // Deploy TTS service
    const tts = await adapter.createProviderInstance("tts-primary", "chatterbox", {
      port: 5000,
      gpuEnabled: true,
      cpuLimit: "2",
      memoryLimit: "4g",
