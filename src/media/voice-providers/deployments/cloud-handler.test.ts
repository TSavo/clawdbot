 ...
 */
function createMockResponse(statusCode: number = 200, data: Buffer = Buffer.alloc(0)): any {
...
describe("CloudHandler", () => {
  ...
  describe("testAuthentication", () => {
    it("should return true for successful auth (200)", async () => {
      ...
      const result = await handler.testAuthentication("http://localhost:8000", "test-key");
      expect(result).toBe(true);
    });
    ...
    it("should return false for auth failure (401)", async () => {
      ...
      const result = await handler.testAuthentication("http://localhost:8000", "bad-key");
      expect(result).toBe(false);
    });
  ...
  describe("synthesize", () => {
    ...
    it("should throw on synthesis failure (500)", async () => {
      const mockResponse = createMockResponse(500, Buffer.from("Internal server error"));
      const mockRequest = createMockRequest();
      ...
      await expect(handler.synthesize("Hello world")).rejects.toThrow(VoiceProviderError);
    });
  ...
  describe("retry logic", () => {
    it("should retry on temporary failure (503)", async () => {
      const failResponse = createMockResponse(503, Buffer.from("Service Unavailable"));
