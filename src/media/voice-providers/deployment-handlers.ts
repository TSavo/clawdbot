...
export class SystemDeploymentHandler {
  ...
  static detectROCm(): { found: boolean; path?: string; version?: string } {
    const rocmPaths = [process.env.ROCM_PATH, "/opt/rocm", "/usr/local/rocm"];
  ...
  static generateInstallScript(config: SystemDeploymentConfig): string {
    ...
    if (config.cudaPath) {
      ...
      lines.push(
        `export LD_LIBRARY_PATH="${path.join(config.cudaPath, "lib64")}:$LD_LIBRARY_PATH"`,
      );
      lines.push("");
...
export async function selectBestDeployment(
  _dockerConfig?: DockerDeploymentConfig,
  _systemConfig?: SystemDeploymentConfig,
