...
export class FasterWhisperService {
  ...
  async initialize(): Promise<ServiceInitResult> {
    ...
    this.log(
      `System: ${systemInfo.cpuModel} (${systemInfo.cpuCount} cores, ${systemInfo.totalMemoryGb}GB RAM)`,
    );
    ...
    try {
      ...
      this.log(`Executor initialization warning: ${errorMsg}`);
    ...
    this.log(
      `Capabilities: ${capabilities.supportedLanguages.length} languages, streaming=${capabilities.supportsStreaming}`,
    );
  ...
  private applyOptimizations(
    recommendations: Awaited<ReturnType<typeof recommendOptimizations>>,
  ): void {
  ...
  async shutdown(): Promise<void> {
    if (this.executor) {
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        this.log(`Shutdown warning: ${errorMsg}`);
      }
    }
