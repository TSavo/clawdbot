...
export class VoiceProviderRegistry {
  ...
  async getTranscriber(providerId?: string): Promise<VoiceProviderExecutor> {
    if (providerId) {
      ...
      throw new VoiceProviderError(`Provider ${providerId} not found`, "registry");
    }

    // Find first available STT provider
    const provider = this.sttProviders[0];
    if (!provider) {
      throw new VoiceProviderError("No STT provider available", "registry");
    }
  ...
  async getSynthesizer(providerId?: string): Promise<VoiceProviderExecutor> {
    ...
    if (!provider) {
      throw new VoiceProviderError("No TTS provider available", "registry");
    }
    ...
    for (let i = 0; i < this.sttProviders.length; i++) {
      ...
      try {
        if (!(await provider.isHealthy())) {
          console.warn(`Provider ${provider.id} is unhealthy, skipping`);
          continue;
        }
        ...
        if (i === this.sttProviders.length - 1) {
          throw new VoiceProviderError("All STT providers failed", "registry");
        }
      }
    }

    throw new VoiceProviderError("No STT providers available", "registry");
  }
    ...
    for (let i = 0; i < this.ttsProviders.length; i++) {
      ...
      try {
        ...
        if (i === this.ttsProviders.length - 1) {
          throw new VoiceProviderError("All TTS providers failed", "registry");
        }
      }
    }

    throw new VoiceProviderError("No TTS providers available", "registry");
  }

  /**
   * Get health status of all providers
   */
  async getHealthStatus(): Promise<Record<string, { healthy: boolean; capabilities?: any }>> {
  ...
  async shutdown(): Promise<void> {
    ...
    if (errors.length > 0) {
      throw new Error(`Failed to shutdown providers: ${errors.map((e) => e.message).join(", ")}`);
    }
  }
   ...
   */
  private async createExecutor(entry: VoiceProviderEntry): Promise<VoiceProviderExecutor> {
    // STT providers
    if (entry.stt) {
      ...
      if (
        sttConfig.type === "openai" ||
        sttConfig.type === "google" ||
        sttConfig.type === "azure"
      ) {
    ...
    if (entry.tts) {
      ...
      if (ttsConfig.type === "local" && ttsConfig.model === "kokoro") {
        ...
        throw new VoiceProviderError("Kokoro executor not yet implemented", entry.id);
      }
      ...
      if (ttsConfig.type === "local" && ttsConfig.model === "piper") {
        ...
        throw new VoiceProviderError("Piper executor not yet implemented", entry.id);
      }
    ...
    throw new VoiceProviderError("Unknown provider configuration", entry.id);
  }
}
