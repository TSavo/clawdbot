 ...
 */
interface _DeepgramStreamConfig {
  model: string;
  language?: string;
  punctuate: boolean;
  tier: "nova" | "enhanced";
...
export class DeepgramExecutor extends BaseVoiceProviderExecutor {
  ...
  constructor(id: string, config: DeepgramConfig) {
    super();
    this.id = id;
  ...
  async initialize(): Promise<void> {
    // Validate API key
    if (!this.apiKey) {
      throw new VoiceProviderError("Deepgram API key is required", this.id, "MISSING_API_KEY");
    }
    ...
    this.healthCheckInterval = setInterval(() => {
      this.performHealthCheck().catch((error) => {
        console.warn(
          `Deepgram health check failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      });
    }, 60000); // Every minute
  }
  ...
  async transcribe(audio: AudioBuffer, options?: TranscribeOptions): Promise<TranscriptionResult> {
    try {
      ...
      if (!audio.data || audio.data.length === 0) {
        throw new VoiceProviderError("Audio data is empty", this.id, "EMPTY_AUDIO");
      }
  ...
  async *transcribeStream(
    ...
    try {
      ...
      let _lastUtteranceTime: number;
      let isSpeaking = false;

      // Setup message handler
      ws.onmessage = (event) => {
        try {
          const message = JSON.parse(String(event.data)) as DeepgramResponse;
          ...
          if (message.result?.results) {
            for (const result of message.result.results) {
              ...
              // Track if any result indicates speech final
              if (result.speech_final) {
                hasAnySpeechFinal = true;
              }
            }
          }

          // Handle VAD events for turn detection
          if (hasAnySpeechFinal) {
            if (isSpeaking) {
              isSpeaking = false;
              // Turn ended
              if (streamMetadata) {
      ...
      while (!connectionClosed) {
        ...
        await Promise.race([closePromise, new Promise<void>((resolve) => setTimeout(resolve, 50))]);
      }
  ...
  getCapabilities(): ProviderCapabilities {
    return {
      supportedFormats: [AudioFormat.PCM_16, AudioFormat.OPUS, AudioFormat.AAC, AudioFormat.MP3],
