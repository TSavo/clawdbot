 ...
 */
function createTestAudioBuffer(duration: number = 1000, sampleRate: number = 16000): AudioBuffer {
...
describe("WhisperDockerDeploymentHandler", () => {
  ...
  beforeEach(async () => {
    ...
    const { WhisperDockerDeploymentHandler } = await import("./whisper.docker.js");
  ...
  describe("Transcription", () => {
    ...
    it("should reject transcription if container not running", async () => {
      const stoppedHandler = new (
        await import("./whisper.docker.js")
      ).WhisperDockerDeploymentHandler();

      const audio = createTestAudioBuffer();

      await expect(stoppedHandler.transcribe(audio)).rejects.toThrow(VoiceProviderError);
    });
    ...
    it("should send transcription request to correct port", async () => {
      ...
      const transcriptionCall = calls.find((call: any[]) => call[0]?.includes("/v1/audio"));
    ...
    it("should handle transcription errors", async () => {
      ...
      await expect(handler.transcribe(audio)).rejects.toThrow(VoiceProviderError);
    });
  ...
  describe("Error Handling", () => {
    ...
    it("should handle missing container ID on startup", async () => {
      ...
      await expect(handler.start()).rejects.toThrow("Failed to start Docker container");
    });
