...
export class FasterWhisperBenchmark {
    ...
    try {
      ...
      for (let i = 0; i < this.config.runs; i++) {
        ...
        try {
          ...
          warnings.push(`Run ${i + 1} failed: ${errorMsg}`);
        }
      }
      ...
      throw new Error(`Benchmark failed for ${computeType}/${beamSize}/${cpuThreads}: ${errorMsg}`);
    }
  }
      ...
      for (const beamSize of beamSizes) {
        for (const threads of cpuThreads) {
          ...
          try {
            ...
            console.log(
              `  Latency: ${result.metrics.avgLatencyMs.toFixed(2)}ms (RTF: ${result.metrics.avgRTF.toFixed(3)})`,
            );
          } catch (_error) {
            const errorMsg = _error instanceof Error ? _error.message : String(_error);
  ...
  private generateReport(
    ...
    const mostAccurateConfig =
      this.results.find((r) => r.computeType === "float32") || fastestConfig;
    ...
    if (systemInfo.totalMemoryGb < 4) {
      recommendations.push(
        "Low memory system: use int8 quantization and beam_size=1 for better performance",
      );
    }
  ...
  static printReport(report: BenchmarkReport): void {
    ...
    console.log(
      `  GPU: ${report.systemInfo.gpuAvailable ? report.systemInfo.gpuType : "Not available"}`,
    );
