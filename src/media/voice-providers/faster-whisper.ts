 ...
 */
export class FasterWhisperExecutor
  extends BaseVoiceProviderExecutor
  implements VoiceProviderExecutor
{
  ...
  override async initialize(): Promise<void> {
    try {
      ...
      console.warn(
        `Failed to initialize Faster-Whisper executor: ${this.initializationError.message}`,
      );
   ...
   */
  async synthesize(_text: string): Promise<AudioBuffer> {
   ...
   */
  async *synthesizeStream(): AsyncIterable<AudioBuffer> {
  ...
  async transcribe(audio: AudioBuffer, options?: TranscribeOptions): Promise<TranscriptionResult> {
    ...
    try {
      ...
      const normalizedAudio = this.normalizeAudioBuffer(audio, AudioFormat.PCM_16, 16000);
      ...
      const result = await this.performTranscription(model, normalizedAudio, transcriptionOpts);
  ...
  getCapabilities(): ProviderCapabilities {
    return {
      supportedFormats: [AudioFormat.PCM_16, AudioFormat.OPUS, AudioFormat.MP3],
  ...
  private async selectOptimalComputeType(): Promise<ComputeType> {
    ...
    if (this.gpuCapabilities?.available) {
      ...
      if (
        this.gpuCapabilities.type === "cuda" &&
        this.gpuCapabilities.memoryGb &&
        this.gpuCapabilities.memoryGb >= 8
      ) {
  ...
  private async loadModel(computeType: ComputeType): Promise<any> {
    ...
    try {
      ...
      const model = await this.createMockModel(this.config.modelSize || "base", computeType);
  ...
  private validateAudioBuffer(audio: AudioBuffer): void {
    if (!audio.data || audio.data.length === 0) {
      throw new VoiceProviderError("Audio buffer is empty", this.id, "INVALID_AUDIO");
    }
