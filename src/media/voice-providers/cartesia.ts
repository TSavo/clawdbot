...
export class CartesiaExecutor extends BaseVoiceProviderExecutor {
    ...
    try {
      ...
      const textStreamPromise = (async () => {
        try {
          ...
          streamError =
            streamError || (error instanceof Error ? error : new Error("Text streaming failed"));
        } finally {
        ...
        if (audioChunks.length > 0) {
          ...
          await Promise.race([
            ...
            new Promise<void>((_resolve, reject) =>
              setTimeout(() => reject(new Error("Audio chunk timeout after 10000ms")), 10000),
            ),
          ]).catch((error) => {
            throw error;
          });
   ...
   */
  private async extractAudioData(response: unknown): Promise<Uint8Array> {
    ...
    if (response && typeof response === "object" && "audio" in response) {
      return this.base64ToArrayBuffer((response as any).audio);
    }
  ...
  private recordMetrics(metrics: SynthesisMetrics): void {
    ...
    this.eventEmitter.emit("synthesis-complete", {
      ...
      firstByteLatencyMs: metrics.firstByteTime ? metrics.firstByteTime - metrics.startTime : 0,
    });
  }
   ...
   */
  async *transcribeStream(_audioStream?: ReadableStream<AudioBuffer>, _options?: TranscribeOptions): AsyncIterable<TranscriptionChunk> {
    // This method is required by the interface but TTS-only providers don't support transcription
    throw new VoiceProviderError(
      "CartesiaAI is a TTS-only provider, transcription not supported",
      this.id,
      "TRANSCRIPTION_NOT_SUPPORTED",
