...
import { DeepgramService, getDeepgramService, resetDeepgramService } from "./deepgram.service.js";
    ...
    results: [
      {
        ...
        words: [{ word: "hello", confidence: 0.92, start: 0.0, end: 0.5 }],
      },
      {
        final: true,
        speech_final: true,
 ...
 */
function createMockAudioBuffer(durationMs: number = 1000, sampleRate: number = 16000): any {
...
describe("DeepgramExecutor", () => {
  ...
  describe("Error Handling", () => {
    ...
    it("should handle HTTP 401 (authentication error)", async () => {
      ...
      await expect(executor.transcribe(createMockAudioBuffer())).rejects.toThrow(
        VoiceProviderError,
      );
    });
    ...
    it("should handle timeout errors", async () => {
      ...
      await expect(executor.transcribe(createMockAudioBuffer(), { timeout: 100 })).rejects.toThrow(
        VoiceProviderError,
      );
    });
...
describe("Deepgram WebSocket Streaming", () => {
      ...
      mockWs._onmessage({
        data: JSON.stringify({
          ...
          result: {
            results: [
              {
                punctuated_result: { transcript: "hello", confidence: 0.95 },
                final: false,
                speech_final: false,
              },
            ],
  ...
  it("should handle WebSocket error gracefully", async () => {
    ...
    const transcribePromise = (async () => {
      try {
        for await (const _chunk of executor.transcribeStream(audioStream)) {
          // Process chunks
        }
      } catch (error) {
