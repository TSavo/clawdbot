...
export function assertValidAudioBuffer(
  ...
  if (expectedFormat && buffer.format !== expectedFormat) {
    throw new Error(`Expected format ${expectedFormat}, got ${buffer.format}`);
  }

  if (expectedDuration && Math.abs(buffer.duration - expectedDuration) > 0.1) {
    throw new Error(`Expected duration ~${expectedDuration}s, got ${buffer.duration}s`);
  }
}
...
export function assertValidTranscriptionResult(
  ...
  if (expectedProvider && result.provider !== expectedProvider) {
    throw new Error(`Expected provider ${expectedProvider}, got ${result.provider}`);
  }

  if (result.confidence !== undefined) {
    if (result.confidence < 0 || result.confidence > 1) {
      throw new Error(`Confidence must be between 0 and 1, got ${result.confidence}`);
    }
  }
}
 ...
 */
export function createProviderConfig(providerIds: string[]): VoiceProvidersConfig {
...
export function extractMetricsFromLogs(
  ...
  for (const log of logs) {
    ...
    if (log.message.includes("failed") || log.message.includes("error")) metrics.failureCount++;
