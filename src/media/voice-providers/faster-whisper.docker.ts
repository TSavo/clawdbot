...
export class FasterWhisperDockerDeploymentHandler {
  ...
  constructor(config: Partial<DockerDeploymentConfig> = {}) {
    this.config = {
      ...
      dockerImage: config.dockerImage || "fedirz/faster-whisper-server:latest-cpu",
  ...
  async start(): Promise<void> {
    try {
      ...
      if (this.config.gpuEnabled) {
        ...
        if (!gpuAvailable) {
          console.warn("GPU requested but not available, falling back to CPU");
  ...
  private async retrieveAssignedPort(): Promise<void> {
    ...
    try {
      const { stdout } = await execAsync(`docker port ${this.containerId} 8000/tcp`);
      ...
      console.warn("Failed to retrieve assigned port, using configured port", error);
   ...
   * Wait for API to be ready
   */
  private async waitForApiReady(maxAttempts: number = 30, delayMs: number = 1000): Promise<void> {
    let attempts = 0;
  ...
  private startHealthCheckInterval(): void {
    this.healthCheckInterval = setInterval(async () => {
      try {
        ...
        if (!healthy) {
          console.warn("Faster-Whisper Docker container health check failed");
  ...
  async transcribe(
    ...
    try {
      ...
      if (options?.computeType || this.config.computeType) {
        formData.append("compute_type", options?.computeType || this.config.computeType);
      }

      if (options?.beamSize !== undefined || this.config.beamSize !== undefined) {
        formData.append("beam_size", String(options?.beamSize ?? this.config.beamSize));
      }

      const response = await fetch(`${this.apiBaseUrl}/v1/audio/transcriptions`, {
        method: "POST",
        body: formData,
  ...
  async getLogs(lines: number = 50): Promise<string> {
    ...
    const { stdout } = await execAsync(`docker logs --tail ${lines} ${this.containerId}`);
