...
import { VoiceOrchestrator, type DeploymentConfig } from "./orchestrator.js";
import {
  AudioFormat,
...
class MockVoiceProvider extends BaseVoiceProviderExecutor {
  ...
  async *synthesizeStream(_textStream: ReadableStream<string>): AsyncIterable<AudioBuffer> {
    yield {
      data: new Uint8Array([0, 1, 2, 3]),
      format: AudioFormat.PCM_16,
      sampleRate: 16000,
...
describe("VoiceOrchestrator", () => {
  ...
  let _mockSTTProvider: MockVoiceProvider;
  let _mockTTSProvider: MockVoiceProvider;
  let config: VoiceProvidersConfig;

  beforeEach(() => {
    _mockSTTProvider = new MockVoiceProvider("mock-stt");
    _mockTTSProvider = new MockVoiceProvider("mock-tts");
  ...
  describe("error handling", () => {
    it("should throw error when transcribing without initialization", async () => {
      ...
      await expect(orchestrator.transcribe(audio)).rejects.toThrow("Orchestrator not initialized");
    });
    ...
    it("should handle provider not found", async () => {
      ...
      expect(() => orchestrator.switchProvider("non-existent")).toThrow("Provider not found");
    });
  });
  ...
  describe("streaming operations", () => {
    it("should handle transcribe stream iteration", async () => {
      ...
      const _mockStream = new ReadableStream<ArrayBuffer>({
        start(controller) {
          controller.close();
        },
      });
