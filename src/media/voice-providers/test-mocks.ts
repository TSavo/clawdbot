...
export class MockSTTProvider extends BaseVoiceProviderExecutor {
  ...
  async transcribe(audio: AudioBuffer, options?: TranscribeOptions): Promise<TranscriptionResult> {
    ...
    switch (this.failureMode) {
      case "timeout":
        throw new VoiceProviderError("Transcription timeout", this.id, "TIMEOUT");
      case "invalid":
        throw new VoiceProviderError("Invalid audio format", this.id, "INVALID_AUDIO");
      case "unhealthy":
        throw new VoiceProviderError("Provider is unhealthy", this.id, "UNHEALTHY");
      default:
        return {
...
export class MockTTSProvider extends BaseVoiceProviderExecutor {
  ...
  async synthesize(text: string, options?: SynthesisOptions): Promise<AudioBuffer> {
    ...
    switch (this.failureMode) {
      case "timeout":
        throw new VoiceProviderError("Synthesis timeout", this.id, "TIMEOUT");
      case "invalid":
        throw new VoiceProviderError("Invalid text content", this.id, "INVALID_TEXT");
      case "unhealthy":
        throw new VoiceProviderError("Provider is unhealthy", this.id, "UNHEALTHY");
      default:
  ...
  async transcribe(_audio: AudioBuffer, _options?: TranscribeOptions): Promise<TranscriptionResult> {
    throw new VoiceProviderError(
...
export class ProviderSimulator {
   ...
   */
  static simulateRateLimiting(provider: MockSTTProvider | MockTTSProvider): void {
