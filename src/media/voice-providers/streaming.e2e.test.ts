...
const TEST_CONFIG = {
  ...
  kokoroEnabled:
    process.env.KOKORO_DOCKER_ENABLED === "true" || process.env.KOKORO_API_URL !== undefined,
...
describe("Kokoro HTTP Streaming E2E", () => {
  ...
  beforeEach(() => {
    if (TEST_CONFIG.kokoroEnabled) {
      ...
      executor = new KokoroExecutor(process.env.KOKORO_API_URL ? cloudConfig : dockerConfig);
    }
  });
