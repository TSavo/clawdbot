...
describe("WhisperExecutor", () => {
  ...
  describe("Initialization", () => {
    ...
    it("should throw error if not initialized", async () => {
      ...
      await expect(executor.transcribe(audio)).rejects.toThrow("not initialized");
    });
  });
  ...
  describe("Audio Buffer Handling", () => {
    it("should handle various audio formats", () => {
      const formats = [AudioFormat.PCM_16, AudioFormat.OPUS, AudioFormat.MP3, AudioFormat.AAC];
...
describe("Audio Stream Handling", () => {
  ...
  it("should support audio streaming interface", async () => {
    ...
    const _stream = new ReadableStream<AudioBuffer>({
      start(controller) {
