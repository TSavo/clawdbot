...
export class FasterWhisperSystemDeploymentHandler {
  ...
  private async checkPythonEnvironment(): Promise<PythonEnvInfo> {
    try {
      ...
      try {
        const { stdout } = await execAsync(`${this.config.pythonPath} -m pip show faster-whisper`);
      ...
      if (!ffmpegAvailable) {
        console.warn("FFmpeg not found - audio format conversion may be limited");
      }
  ...
  private async verifyDependencies(): Promise<void> {
    ...
    if (missingDeps.length > 0) {
      console.warn(`Missing Python dependencies: ${missingDeps.join(", ")}`);
  ...
  private async ensureModelAvailable(): Promise<void> {
    ...
    console.log(`Downloading Faster-Whisper model: ${this.config.modelSize}...`);
...
`;
  ...
  async getModelInfo(): Promise<ModelInfo> {
    const modelPath = join(this.config.cachePath, `faster-whisper-${this.config.modelSize}`);
  ...
  private detectDevice(): "cpu" | "cuda" | "mps" {
    // Try to detect CUDA
    try {
      void execAsync("nvidia-smi").catch(() => {
        // Not CUDA
      });
      return "cuda";
    } catch {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(
        () => {
          childProcess.kill();
          reject(
            new VoiceProviderError("Transcription timeout", "faster-whisper-system", "TIMEOUT"),
          );
        },
   ...
   */
  private generateTranscriptionScript(audio: AudioBuffer, options?: TranscribeOptions): string {
