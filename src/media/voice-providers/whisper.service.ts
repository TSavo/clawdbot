...
export class WhisperPluginService {
   ...
   */
  async initialize(providerId: string, config: z.infer<typeof WhisperConfigSchema>): Promise<void> {
    try {
   ...
   * Restart the service
   */
  async restart(providerId: string, config: z.infer<typeof WhisperConfigSchema>): Promise<void> {
    try {
 ...
 */
export function getWhisperPluginService(config?: WhisperPluginServiceConfig): WhisperPluginService {
  if (!globalService) {
