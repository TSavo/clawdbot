...
class MockWebSocket {
  ...
  constructor(
    public url: string,
    options?: { messageDelay?: number; chunksPerText?: number },
  ) {
...
describe("CartesiaExecutor WebSocket Streaming", () => {
  ...
  describe("latency and performance", () => {
    it("should receive first chunk in <100ms", async () => {
      ...
      for await (const _chunk of executor.synthesizeStream(textStream)) {
        if (chunkCount === 0) {
    ...
    it("should maintain consistent chunk arrival times", async () => {
      ...
      const avgInterval =
        arrivalTimes.length > 1
          ? (arrivalTimes[arrivalTimes.length - 1] - arrivalTimes[0]) / (arrivalTimes.length - 1)
          : 0;

      console.log(
        `Avg chunk interval: ${avgInterval.toFixed(2)}ms over ${arrivalTimes.length} chunks`,
      );
