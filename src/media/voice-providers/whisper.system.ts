...
export class WhisperSystemDeploymentHandler {
  ...
  private async checkPythonEnvironment(): Promise<PythonEnvInfo> {
    try {
      ...
      try {
        const { stdout } = await execAsync(`${this.config.pythonPath} -m pip show openai-whisper`);
...
`;
  ...
  async getModelInfo(): Promise<ModelInfo> {
    const modelPath = join(this.config.cachePath, `${this.config.modelSize}.pt`);
  ...
  async healthCheck(): Promise<boolean> {
    ...
    try {
      ...
      await execAsync(`${this.config.pythonPath} -c "import whisper; print('ok')"`);
      return true;
    } catch {
      return false;
    }
  ...
  async transcribe(audio: AudioBuffer, options?: TranscribeOptions): Promise<TranscriptionResult> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(
        () => {
          proc.kill();
          reject(new VoiceProviderError("Transcription timeout", "whisper-system", "TIMEOUT"));
        },
