...
export class VoiceOrchestrator {
  ...
  async transcribe(audio: AudioBuffer, options?: TranscribeOptions): Promise<TranscriptionResult> {
    ...
    if (this.sttProviders.length === 0) {
      throw new VoiceProviderError("No STT providers available", "orchestrator", "NO_PROVIDERS");
    }
  ...
  private async transcribeWithProvider(
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      throw new VoiceProviderError(`Transcription error: ${errorMsg}`, provider.id);
    }
  }
  ...
  async synthesize(text: string, options?: SynthesisOptions): Promise<AudioBuffer> {
    ...
    if (this.ttsProviders.length === 0) {
      throw new VoiceProviderError("No TTS providers available", "orchestrator", "NO_PROVIDERS");
    }
  ...
  private async synthesizeWithProvider(
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      throw new VoiceProviderError(`Synthesis error: ${errorMsg}`, provider.id);
    }
  }
  ...
  switchProvider(id: string): void {
    ...
    const currentEntry = this.config.providers?.find((p) => this.providers.get(p.id) === provider);
    if (!currentEntry) {
      throw new VoiceProviderError(
  ...
  private async loadProviders(): Promise<void> {
    ...
    for (const entry of ordered) {
      ...
      try {
        ...
        this.logger.warn(`Failed to load provider ${entry.id}`, {
          error: error instanceof Error ? error.message : String(error),
        });
      }
    }
   ...
   */
  private async createProviderExecutor(entry: VoiceProviderEntry): Promise<VoiceProviderExecutor> {
