...
export class FasterWhisperPluginService {
  ...
  constructor(config: FasterWhisperPluginServiceConfig = {}) {
    this.config = {
      ...
      cacheDir: config.cacheDir || `${process.env.HOME}/.cache/faster-whisper`,
   ...
   */
  async initialize(providerId: string, config: FasterWhisperConfig): Promise<void> {
    try {
      ...
      if (!healthy) {
        console.warn("Faster-Whisper executor is not healthy after initialization");
      }
    } catch (error) {
   ...
   * Restart the service
   */
  async restart(providerId: string, config: FasterWhisperConfig): Promise<void> {
    try {
