...
export class ElevenLabsExecutor extends BaseVoiceProviderExecutor {
    ...
    async initialize() {
        ...
        if (!this.config.apiKey) {
            throw new VoiceProviderError('ElevenLabs API key is required', this.id, 'MISSING_API_KEY');
        }
        try {
            ...
            throw new VoiceProviderError(`Failed to initialize ElevenLabs: ${error instanceof Error ? error.message : String(error)}`, this.id, 'INITIALIZATION_FAILED');
        }
    }
    async shutdown() {
    ...
    async transcribe(_audioBuffer, _options) {
        throw new VoiceProviderError('Transcription is not supported by ElevenLabs TTS provider', this.id, 'NOT_SUPPORTED');
    }
    async *transcribeStream(_audioStream, _options) {
        throw new VoiceProviderError('Streaming transcription is not supported by ElevenLabs TTS provider', this.id, 'NOT_SUPPORTED');
        // yield to make it an async generator
    }
    async synthesize(text, options) {
        if (!this.isInitialized) {
            throw new VoiceProviderError('ElevenLabs executor not initialized', this.id, 'NOT_INITIALIZED');
        }
        if (!text || text.trim().length === 0) {
            throw new VoiceProviderError('Text is required for synthesis', this.id, 'INVALID_INPUT');
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new VoiceProviderError(`ElevenLabs API error: ${errorText}`, this.id, `HTTP_${response.status}`);
            }
    ...
    async *synthesizeStream(textStream, _options) {
        if (!this.isInitialized) {
            throw new VoiceProviderError('ElevenLabs executor not initialized', this.id, 'NOT_INITIALIZED');
        }
        try {
            ...
            if (error instanceof VoiceProviderError)
                throw error;
            throw new VoiceProviderError(`Stream synthesis failed: ${error instanceof Error ? error.message : String(error)}`, this.id, 'STREAM_SYNTHESIS_FAILED');
        }
    }
    getCapabilities() {
        return {
    ...
    async healthCheck() {
        if (!this.config.apiKey) {
            throw new VoiceProviderError('API key not configured', this.id, 'MISSING_API_KEY');
        }
        ...
        if (!response.ok) {
            throw new VoiceProviderError(`Health check failed: ${response.statusText}`, this.id, `HTTP_${response.status}`);
        }
    }
...
//# sourceMappingURL=tts-elevenlabs.js.map