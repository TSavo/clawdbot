...
async function promptTwilioCallConfig(prompter: WizardPrompter): Promise<TwilioCallConfig> {
  ...
  if (!validateTwilioAccountSid(accountSid)) {
    throw new Error(
      "Invalid Twilio Account SID format (must be ACxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx)",
    );
  }
...
export async function setupVoiceCallProviders(params: {
  ...
  if (providers.twilio) {
    ...
    validations.push("Note: Twilio credentials will be validated when the bot connects");
  }

  if (validations.length > 0) {
    await prompter.note(
      ["Configuration validated:", ...validations.map((v) => `  ✓ ${v}`)].join("\n"),
      "Validation complete",
    );
  }
...
export async function testVoiceCallProviders(params: {
  ...
  const hasCallsConfig =
    callsConfig?.enabled && Object.keys(callsConfig?.providers || {}).length > 0;
