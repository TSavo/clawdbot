...
async function installVoiceProviderPlugin(
  ...
  _options?: { verbose?: boolean; cacheDir?: string },
 ...
 */
async function promptDockerConfiguration(
  ...
  const dockerImages: Record<string, string[]> = {
    stt: ["whisper:latest", "faster-whisper:latest", "whisper:small", "faster-whisper:small"],
    tts: ["kokoro:latest", "piper:latest", "kokoro:v1", "piper:v1"],
  };
 ...
 */
async function promptCloudTTSProvider(prompter: WizardPrompter): Promise<TTSProviderConfig> {
  const service = (await prompter.select({
...
async function initializeSystemModeDeployment(
  ...
  try {
    await prompter.note(
      "Installing system dependencies (ffmpeg, python3, models)...",
      "Installing",
    );
    ...
    if (result.success) {
      ...
      await prompter.note(
        ["✓ System mode initialized successfully!", "", "Installed components:", details].join(
          "\n",
        ),
...
export async function setupVoiceProviders(params: {
  ...
  if (setupSTT) {
    ...
    if (sttType === "system") {
      ...
      const { model, modelSize, computeType, cpuThreads, beamSize } = await promptLocalSTTModel(
        prompter,
        capabilities,
      );
  ...
  if (setupTTS) {
    ...
    if (ttsType === "system") {
      ...
      // Initialize system mode (install dependencies automatically)
      const initSuccess = await initializeSystemModeDeployment(prompter, model, { voice }, false);
...
export async function testVoiceProviders(params: {
  ...
  const hasVoiceConfig =
    cfg.voice?.providers?.providers && cfg.voice.providers.providers.length > 0;
  ...
  await prompter.note(
    "Voice provider testing is available from the CLI using: clawdbot voice test",
    "Voice test",
  );
