...
import { intro, outro, confirm, select } from "./configure.shared.js";
 ...
 */
export async function voiceProvidersCommand(_runtime?: RuntimeEnv): Promise<void> {
 ...
 */
export async function voiceTestCommand(_runtime?: RuntimeEnv): Promise<void> {
  try {
    const snapshot = await readConfigFileSnapshot();
    const config = snapshot.config;
    ...
    const msg = error instanceof Error ? error.message : String(error);
    _runtime?.error?.(`Failed to test voice providers: ${msg}`);
  }
}
