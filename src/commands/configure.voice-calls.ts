...
async function promptVoiceCallsMode(_prompter: WizardPrompter): Promise<VoiceCallsMode> {
...
async function listVoiceCallProviders(
  ...
  if (lines.length === 0) {
    ...
    await prompter.note(["Enabled voice call providers:", ...lines].join("\n"), "Voice calls");
  }
}
...
async function configureDiscord(
  ...
  const guildId = await text({
    ...
    validate: (value) => {
      ...
      return !trimmed || validateDiscordGuildId(trimmed)
        ? undefined
        : "Invalid Guild ID format (18-19 digits)";
    },
  });
  ...
  const participantLimit =
    participantLimitStr && !isNaN(Number(participantLimitStr))
...
async function configureTelegram(
  ...
  const groupId = await text({
    ...
    validate: (value) => {
      const trimmed = String(value || "").trim();
      return !trimmed || validateTelegramGroupId(trimmed)
        ? undefined
        : "Invalid group ID format (must be negative)";
    },
  });
  ...
  const maxParticipants =
    maxParticipantsStr && !isNaN(Number(maxParticipantsStr))
...
async function configureSignal(
  ...
  const phoneNumber = await text({
    ...
    validate: (value) => {
      const trimmed = String(value || "").trim();
      return !trimmed || validateE164PhoneNumber(trimmed)
        ? undefined
...
async function configureTwilio(
  ...
  const accountSid = await text({
    ...
    validate: (value) => {
      const trimmed = String(value || "").trim();
      return !trimmed || validateTwilioAccountSid(trimmed)
        ? undefined
  ...
  const defaultTimeoutSeconds =
    timeoutStr && !isNaN(Number(timeoutStr))
