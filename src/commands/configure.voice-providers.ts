...
async function promptVoiceProviderMode(_prompter: WizardPrompter): Promise<VoiceProviderMode> {
...
async function listVoiceProviders(
  ...
  await prompter.note(["Configured voice providers:", ...lines].join("\n"), "Voice providers");
}

async function promptSTTConfig(
  prompter: WizardPrompter,
  current?: STTProviderConfig,
): Promise<STTProviderConfig | null> {
...
async function promptTTSConfig(
  prompter: WizardPrompter,
  current?: TTSProviderConfig,
