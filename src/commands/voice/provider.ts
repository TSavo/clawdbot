 ...
 */
async function showProviderStatus(_runtime: RuntimeEnv): Promise<void> {
  const spin = spinner();
  ...
  for (const provider of voiceConfig.providers) {
    ...
    const statusIcon = status?.healthy ? "✓ Healthy" : "✗ Unhealthy";
    ...
    rows.push([provider.id, typeInfo.join("/"), statusIcon, provider.priority || 1]);
  }

  printTable(["Provider ID", "Type", "Status", "Priority"], rows);
