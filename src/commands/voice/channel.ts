 ...
 */
async function listChannels(_runtime: RuntimeEnv = defaultRuntime): Promise<void> {
  ...
  for (const [id, channel] of channels) {
    ...
    rows.push([id, stats.participantCount, activeSpeakers, stats.isActive ? "Active" : "Inactive"]);
  }

  printTable(["Channel ID", "Participants", "Active Speakers", "Status"], rows);
}

/**
 * Show channels summary
 */
async function showChannelsSummary(_runtime: RuntimeEnv = defaultRuntime): Promise<void> {
  ...
  for (const [id, channel] of channels) {
    ...
    channelInfo.push(`  ${id}: ${stats.participantCount} participants`);
  }
...
async function showChannelStatus(
  ...
  if (stats.participantCount > 0) {
    ...
    for (const [userId, participant] of channel.participants) {
      ...
      console.log(`  ${status} ${participant.displayName} (${userId})`);
    }
  }
}
