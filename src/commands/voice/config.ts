 ...
 */
async function listProviders(_runtime: RuntimeEnv): Promise<void> {
  const spin = spinner();
 ...
 */
async function showSummary(_runtime: RuntimeEnv): Promise<void> {
  const { voiceConfig } = await loadVoiceConfig();
 ...
 */
async function showDeployment(_runtime: RuntimeEnv): Promise<void> {
  const spin = spinner();
  ...
  for (const provider of voiceConfig.providers) {
    const executor =
      (await registry.getTranscriber(provider.id).catch(() => null)) ||
 ...
 */
async function editProvider(providerId: string, _runtime: RuntimeEnv): Promise<void> {
