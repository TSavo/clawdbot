...
import type { ClawdbotConfig } from "../../config/types.clawdbot.js";
...
export async function loadVoiceConfig(): Promise<{
  ...
  const voiceConfig: VoiceProvidersConfig = (fullConfig.voice?.providers as
...
export function formatCapabilities(caps: any): string[] {
  ...
  if (caps.supportedLanguages) {
    lines.push(
      `Languages: ${caps.supportedLanguages.slice(0, 3).join(", ")}${caps.supportedLanguages.length > 3 ? "..." : ""}`,
    );
  }
