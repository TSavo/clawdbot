...
export async function transcribeCommand(
  ...
  try {
    ...
    if (!isAudioFile(opts.file)) {
      throw new Error(`Audio file not found or unsupported format: ${opts.file}`);
    }

    // Load configuration
    const { voiceConfig } = await loadVoiceConfig();
    if (!voiceConfig?.enabled || !voiceConfig.providers?.length) {
      throw new Error('Voice providers not configured. Run "clawdbot configure voice" first.');
    }
    ...
    spin.stop(`Audio loaded: ${(audioData.length / 1024).toFixed(2)}KB`);
