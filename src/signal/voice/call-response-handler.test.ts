...
import { signalRpcRequest as _signalRpcRequest } from "../client.js";
...
describe("SignalVoiceCallResponseHandler", () => {
  ...
  describe("audio streaming", () => {
    ...
    it("should start audio streaming", async () => {
      ...
      vi.mocked(_signalRpcRequest).mockResolvedValue({
        chunks: [],
        hasMore: false,
      });
    ...
    it("should emit audio received events", async () => {
      ...
      // Mock audio chunks
      vi.mocked(_signalRpcRequest).mockResolvedValueOnce({
        chunks: [
          {
  ...
  describe("transcription", () => {
    ...
    it("should transcribe audio chunks", async () => {
      ...
      const mockTranscribe = vi.mocked(mockTranscriptionProvider.transcribe);
      expect(mockTranscribe.mock.calls.length).toBeGreaterThan(0);
  ...
  describe("agent responses", () => {
    ...
    it("should generate agent response from transcription", async () => {
      ...
      // eslint-disable-next-line typescript-eslint/unbound-method
      const mockSynthesize = vi.mocked(mockTtsProvider.synthesize);
    ...
    it("should handle TTS synthesis errors gracefully", async () => {
      ...
      // Mock TTS failure
      // eslint-disable-next-line typescript-eslint/unbound-method
      const mockSynthesizeReject = vi.mocked(mockTtsProvider.synthesize);
      mockSynthesizeReject.mockRejectedValueOnce(
  ...
  describe("privacy", () => {
    ...
    it("should clear memory on stream end", async () => {
      ...
      const logCalls = (mockRuntime.log as any).mock.calls;
      const hasMemoryClearLog = logCalls.some((call: any[]) => call[0]?.includes("memory cleared"));
