...
export class SignalVoiceCallResponseHandler extends EventEmitter {
  ...
  constructor(rpcOptions: SignalRpcOptions, config: CallResponseConfig = {}, runtime?: RuntimeEnv) {
    super();
  ...
  async startStreaming(call: VoiceCall, streamId: string): Promise<void> {
    ...
    this.runtime?.log?.(`Starting audio streaming for call ${call.callId} (stream: ${streamId})`);

    // Start receiving audio
    void this.startReceivingAudio();
  }
  ...
  private async processInboundAudio(chunk: AudioChunk): Promise<void> {
    ...
    try {
      ...
      const transcription = await this.transcriptionProvider.transcribe(audioBuffer, {
        language: "en",
      ...
      if (transcription.text.trim()) {
        ...
        if (this.transcriptionMemory.length > this.config.agentContextWindow * 2) {
          this.transcriptionMemory = this.transcriptionMemory.slice(
            -this.config.agentContextWindow * 2,
          );
        }
  ...
  private async generateAgentResponse(userInput: string): Promise<void> {
    try {
      ...
      const _recentTranscriptions = this.transcriptionMemory
  ...
  private async streamEncryptedResponse(audioBuffer: AudioBuffer): Promise<void> {
    ...
    try {
      ...
      this.runtime?.log?.(`Streamed ${chunks.length} audio chunks in ${totalStreamLatency}ms`);
    } catch (error) {
      this.runtime?.error?.(
