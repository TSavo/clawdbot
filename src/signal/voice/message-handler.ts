...
export async function downloadVoiceMessage(
  ...
  const result = await signalRpcRequest<{ data?: string }>("getAttachment", params, rpcOptions);
...
export async function saveVoiceMessageAudio(
  ...
  const saved = await saveMediaBuffer(message.audioBuffer, contentType, subdir, maxBytes);
...
export function extractVoiceMetadata(message: DecryptedVoiceMessage): VoiceMessageMetadata {
  return {
