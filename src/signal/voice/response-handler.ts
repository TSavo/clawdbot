...
async function convertToSignalFormat(
  ...
  _targetFormat: "opus" | "wav" | "ogg" = "opus",
): Promise<Buffer> {
  ...
  throw new Error(`Audio format ${audio.format} not supported. Convert to OPUS/WAV/OGG first.`);
}
...
export async function sendEncryptedVoiceMessage(params: {
  ...
  const { recipient, audio, baseUrl, account, accountId, groupId, caption, config, runtime } =
    params;
  ...
  try {
    ...
    runtime?.error?.(`Failed to send voice message: ${errorMsg}`);
    throw error;
  }
}
...
export async function handleVoiceResponse(params: {
  ...
  const { text, recipient, groupId, baseUrl, account, accountId, ttsExecutor, config, runtime } =
    params;
  ...
  const audio = await synthesizeTextToVoice(text, responseConfig, ttsExecutor, runtime);
...
export async function sendVoiceReaction(params: {
  ...
  const { targetMessageId, targetAuthor, emoji, baseUrl: _baseUrl, account: _account, groupId: _groupId, runtime } = params;

  // TODO: Implement Signal reaction API
  // Signal supports emoji reactions on messages
  runtime?.log?.(`Sending reaction ${emoji} to message ${targetMessageId} from ${targetAuthor}`);
