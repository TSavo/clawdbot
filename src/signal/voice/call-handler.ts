...
export class SignalVoiceCallHandler extends EventEmitter {
  ...
  constructor(rpcOptions: SignalRpcOptions, config: CallHandlerConfig = {}, runtime?: RuntimeEnv) {
    super();
  ...
  private isCallerAllowed(caller: string, callerUuid?: string): boolean {
    ...
    return (
      this.config.allowedCallers.includes(caller) ||
  ...
  async handleIncomingCall(params: {
    ...
    this.runtime?.log?.(`Incoming ${callType} call from ${caller} (ID: ${callId})`);
  ...
  async acceptCall(account?: string): Promise<void> {
    ...
    try {
      ...
      this.runtime?.log?.(`Call accepted and connected (E2E verified: ${call.encryptionVerified})`);
  ...
  async endCall(reason?: string): Promise<void> {
    ...
    try {
      ...
      this.runtime?.log?.(`Call ended (duration: ${(call.duration / 1000).toFixed(1)}s)`);
    ...
    this.callMonitorInterval = setInterval(async () => {
      ...
      try {
        const qualityResponse = await signalRpcRequest<{
          quality?: string;
          latencyMs?: number;
        }>("getCallQuality", { callId: call.callId }, this.rpcOptions);
