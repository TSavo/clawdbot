...
export function registerVoiceCommands(program: Command): void {
  const voiceProgram = program.command("voice").description("Voice provider operations (STT/TTS)");
  ...
  voiceProgram
    ...
    .action(async (opts) => {
      const hasSubcommand =
        opts.list || opts.setDefaultStt || opts.setDefaultTts || opts.showDeployment || opts.edit;
  ...
  voiceProgram
    ...
    .action(async (opts) => {
      const hasSubcommand =
        opts.create || opts.list || opts.add || opts.remove || opts.status || opts.delete;

      if (!hasSubcommand) {
        opts.list = true;
      }

      const maxParticipants = opts.maxParticipants ? parseInt(opts.maxParticipants, 10) : undefined;
