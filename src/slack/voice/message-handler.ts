...
import type { AudioFormat } from "../../media/voice-providers/executor.js";
...
const AUDIO_FILE_EXTENSIONS = [".mp3", ".wav", ".ogg", ".opus", ".m4a", ".aac", ".webm"];
...
export class SlackVoiceMessageHandler {
   ...
   */
  private async detectAudioFormat(buffer: Buffer, fileName: string): Promise<AudioFormat> {
    ...
    try {
      ...
      this.runtime.error?.(`Failed to detect file type: ${err instanceof Error ? err.message : String(err)}`);
    }
   ...
   */
  async downloadAudioFile(file: SlackVoiceFile, channelId: string): Promise<SlackVoiceContext> {
  ...
  private async _downloadAudioFileImpl(
    ...
    if (file.size > this.maxSize) {
      throw new Error(`Audio file too large: ${file.size} bytes (max: ${this.maxSize})`);
    }
    ...
    this.runtime.log?.(
      `Downloaded voice message: ${file.name} (${format}, ${buffer.length} bytes)`,
    );
  ...
  async cleanup(context: SlackVoiceContext): Promise<void> {
    try {
      ...
      this.runtime.error?.(`Failed to cleanup voice file: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
  ...
  async cleanupOldFiles(maxAgeMs: number = 3600000): Promise<void> {
    try {
      ...
      this.runtime.error?.(`Failed to cleanup old files: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}
