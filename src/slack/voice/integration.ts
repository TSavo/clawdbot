 ...
 */
export function hasVoiceAttachment(message: {
  files?: Array<{ mimetype?: string; name?: string }>;
}): boolean {
...
export async function handleSlackVoiceMessage(params: {
   ...
   */
  sendFn: (params: { voiceBuffer?: Buffer; text?: string; threadTs?: string }) => Promise<void>;
  threadTs?: string;
}): Promise<boolean> {
  const {
    message,
  ...
  try {
    ...
    const voiceContext: SlackVoiceContext = await messageHandler.downloadAudioFile(
      voiceFile,
      channelId,
    );

    logVerbose(
      `slack-voice: downloaded audio (format: ${voiceContext.format}, size: ${voiceContext.sizeBytes} bytes)`,
    );
 ...
 */
export function shouldHandleSlackVoiceMessage(message: {
  files?: Array<{ mimetype?: string; name?: string }>;
}): boolean {
