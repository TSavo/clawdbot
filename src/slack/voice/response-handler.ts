...
export class SlackVoiceResponseHandler {
   ...
   */
  private determineOutputModality(options: SlackVoiceResponseOptions): {
    ...
    const configuredModality =
      this.slackVoiceConfig.messageResponse ?? this.slackVoiceConfig.defaultResponse ?? "match";
   ...
   */
  private async sendVoiceOnly(options: SlackVoiceResponseOptions): Promise<SlackVoiceUploadResult> {
    // Synthesize speech
   ...
   */
  private async synthesizeSpeech(text: string, options: SynthesisOptions): Promise<AudioBuffer> {
    try {
  ...
  async cleanupOldFiles(maxAgeMs: number = 3600000): Promise<void> {
    try {
      ...
      this.runtime.error?.(`Failed to cleanup old response files: ${err instanceof Error ? err.message : String(err)}`);
    }
  }
}
