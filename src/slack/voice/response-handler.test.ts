...
describe("SlackVoiceResponseHandler", () => {
  ...
  describe("cleanupFile", () => {
    it("should remove temporary file", async () => {
      ...
      await expect(import("fs/promises").then((fs) => fs.access(testFile))).rejects.toThrow();
    });
    ...
    it("should ignore errors when file does not exist", async () => {
      ...
      // Should not throw
      await expect((handler as any).cleanupFile(nonExistent)).resolves.toBeUndefined();
    });
  });
  ...
  describe("cleanupOldFiles", () => {
    it("should remove old response files", async () => {
      ...
      await expect(import("fs/promises").then((fs) => fs.access(oldFile1))).rejects.toThrow();
      await expect(import("fs/promises").then((fs) => fs.access(oldFile2))).rejects.toThrow();
    });
  });
  ...
  describe("configuration", () => {
    ...
    it("should accept custom configuration", () => {
      const customHandler = new SlackVoiceResponseHandler(mockClient, mockRuntime, {
        defaultVoice: "en_uk",
        defaultSpeed: 1.5,
