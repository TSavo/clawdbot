...
describe("SlackVoiceMessageHandler", () => {
  ...
  describe("downloadAudioFile", () => {
    it("should reject files that are too large", async () => {
      ...
      await expect(handler.downloadAudioFile(file, "C123")).rejects.toThrow("Audio file too large");
    });
    ...
    it("should reject files without download URL", async () => {
      ...
      await expect(handler.downloadAudioFile(file, "C123")).rejects.toThrow(
  ...
  describe("cleanupOldFiles", () => {
    it("should remove old voice files", async () => {
      ...
      await expect(import("fs/promises").then((fs) => fs.access(oldFile))).rejects.toThrow();
