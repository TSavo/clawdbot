 ...
 */
export async function setupSlackVoiceIntegration(app: App, client: WebClient, runtime: RuntimeEnv) {
  ...
  registerSlackVoiceEvents({
    ...
    handlers: {
      messageHandler,
      responseHandler,
      onVoiceMessage: async (fileId: string, channelId: string, userId?: string) => {
        ...
        try {
          ...
          runtime.log?.(`Downloaded voice file: ${context.audioPath} (${context.format})`);
          ...
          runtime.error?.(`Voice message handling failed: ${err instanceof Error ? err.message : String(err)}`);
        }
      },
    },
  });
 ...
 */
async function processVoiceWithAgent(_context: any): Promise<string> {
 ...
 * - Send both formats
 */
export async function setupModalityMatching(client: WebClient, runtime: RuntimeEnv) {
