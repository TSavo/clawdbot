...
export async function handleTelegramVoiceMessage(params: {
   ...
   */
  sendFn: (params: { voiceBuffer?: Buffer; text?: string; replyToId?: number }) => Promise<void>;
}): Promise<boolean> {
  const { message, bot, token, providersConfig, chatId: _chatId, userId: _userId, replyFn, sendFn } = params;
