...
export interface TTSProvider {
  ...
  synthesize(text: string, options?: { voiceId?: string; sampleRate?: number }): Promise<Buffer>;
}

export interface AudioEncoder {
  /** Encode PCM audio to Ogg Opus format */
  encodeToOggOpus(pcmBuffer: Buffer, sampleRate: number, channels?: number): Promise<Buffer>;
}
 ...
 */
export function createVoiceResponseHandler(options: VoiceResponseOptions): VoiceResponseHandler {
 ...
 */
export function chunkTextForVoice(text: string, maxChunkLength: number = 500): string[] {
  // Split by sentences
