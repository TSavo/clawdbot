...
describe("sendVoiceResponse", () => {
  ...
  it("should send voice response successfully", async () => {
    const result = await sendVoiceResponse("Hello, world!", { chatId: 456 }, baseOptions);

    expect(result.success).toBe(true);
    expect(result.messageId).toBe(123);
    // eslint-disable-next-line typescript-eslint/unbound-method
    expect(vi.mocked(mockTTSProvider.synthesize)).toHaveBeenCalledWith("Hello, world!", {
      voiceId: "test-voice",
      sampleRate: 48000,
    });
    // eslint-disable-next-line typescript-eslint/unbound-method
    expect(vi.mocked(mockAudioEncoder.encodeToOggOpus)).toHaveBeenCalled();
    expect(vi.mocked(mockBotApi.sendVoice)).toHaveBeenCalled();
  });

  it("should reject empty text", async () => {
    const result = await sendVoiceResponse("", { chatId: 456 }, baseOptions);
  ...
  it("should reject text exceeding max length", async () => {
    ...
    const result = await sendVoiceResponse(longText, { chatId: 456 }, baseOptions);
  ...
  it("should use custom voice ID and sample rate", async () => {
    ...
    // eslint-disable-next-line typescript-eslint/unbound-method
    // eslint-disable-next-line typescript-eslint/unbound-method
    expect(vi.mocked(mockTTSProvider.synthesize)).toHaveBeenCalledWith("Test message", {
      voiceId: "custom-voice",
      sampleRate: 24000,
    });
  });
...
describe("VoiceResponseHandler", () => {
  ...
  it("should update TTS provider", async () => {
    ...
    await handler.sendVoice("Test", { chatId: 456 });
    // eslint-disable-next-line typescript-eslint/unbound-method
    expect(vi.mocked(newProvider.synthesize)).toHaveBeenCalled();
  });
  ...
  it("should update default voice settings", async () => {
    ...
    await handler.sendVoice("Test", { chatId: 456 });
    // eslint-disable-next-line typescript-eslint/unbound-method
    expect(vi.mocked(freshTTSProvider.synthesize)).toHaveBeenCalledWith("Test", {
      voiceId: "new-voice",
      sampleRate: 24000,
    });
  });
...
describe("sendLongVoiceResponse", () => {
  ...
  it("should send single chunk for short text", async () => {
    const result = await sendLongVoiceResponse("Short message.", { chatId: 456 }, baseOptions);
  ...
  it("should send multiple chunks for long text", async () => {
    ...
    const result = await sendLongVoiceResponse(longText, { chatId: 456 }, baseOptions, 50);
  ...
  it("should only reply to first chunk", async () => {
    ...
    await sendLongVoiceResponse(longText, { chatId: 456, replyToMessageId: 789 }, baseOptions, 20);
  ...
  it("should collect errors from failed chunks", async () => {
    ...
    const longText = "A. ".repeat(100);
    const result = await sendLongVoiceResponse(longText, { chatId: 456 }, baseOptions, 20);
...
describe("determineResponseModality", () => {
  ...
  it("should return voice when input is voice with match modality", () => {
    ...
    const modality = determineResponseModality({ chatId: 456, inputModality: "voice" }, options);
  ...
  it("should return text when input is text with match modality", () => {
    ...
    const modality = determineResponseModality({ chatId: 456, inputModality: "text" }, options);
  ...
  it("should respect match modality in per-chat override", () => {
    ...
    const modalityText = determineResponseModality({ chatId: 456, inputModality: "text" }, options);
...
describe("VoiceResponseHandler modality methods", () => {
  ...
  it("should send text response", async () => {
    ...
    expect(mockBotApi.sendMessage).toHaveBeenCalledWith(456, "Hello text", expect.any(Object));
  });
