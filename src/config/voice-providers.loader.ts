 ...
 */
export function loadVoiceProvidersConfig(cfg: ClawdbotConfig): {
    ...
    try {
      ...
      if (provider.stt) {
        if (provider.stt.type === "whisper" || provider.stt.type === "faster-whisper") {
          ...
          provider.stt.type === "openai" ||
          provider.stt.type === "google" ||
          provider.stt.type === "azure"
        ) {
          // Cloud STT provider (openai, google, azure)
          if (!provider.stt.apiKey) {
            result.warnings.push(
              `Cloud STT (${provider.stt.service || provider.stt.type}) missing API key`,
            );
            if (!options?.validateOnly) {
              result.success = false;
      ...
      if (provider.tts) {
        if (
          provider.tts.type === "local" ||
          provider.tts.type === "kokoro" ||
          provider.tts.type === "piper"
        ) {
          const { checkPackageInstalled } = await import("./voice-providers.utils.js");
          const installed = checkPackageInstalled(
            provider.tts.type === "kokoro" ? "kokoro" : "piper",
          );
          ...
          if (!provider.tts.apiKey) {
            result.warnings.push(
              `Cloud TTS (${provider.tts.service || provider.tts.type}) missing API key`,
            );
            if (!options?.validateOnly) {
              result.success = false;
      ...
      if (result.success) {
        const sttStatus = result.details.sttStatus?.toString() || "N/A";
        const ttsStatus = result.details.ttsStatus?.toString() || "N/A";
        logger.info(
          `Provider ${provider.id} initialized successfully (STT: ${sttStatus}, TTS: ${ttsStatus})`,
        );
      }
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
