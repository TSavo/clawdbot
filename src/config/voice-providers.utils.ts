  ...
  if (process.platform === "darwin") {
    ...
    try {
      const result = execSync(
        "system_profiler SPDisplaysDataType 2>/dev/null | grep -i gpu || true",
        {
          encoding: "utf-8",
          timeout: 2000,
        },
...
export function validateSTTProviderConfig(config: Record<string, unknown>): {
  ...
  if (providerType === "whisper") {
    ...
    if (
      config.beamSize &&
      (typeof config.beamSize !== "number" || config.beamSize < 1 || config.beamSize > 512)
    ) {
      errors.push("beamSize must be between 1 and 512");
    }
    if (
      config.computeType &&
      !["int8", "float16", "float32"].includes(config.computeType as string)
    ) {
