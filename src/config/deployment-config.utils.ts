 ...
 */
export function validateDeploymentConfig(config: unknown): {
  valid: boolean;
  ...
  try {
    ...
    switch (mode) {
      ...
      default: {
        ...
        return { valid: false, errors: [`Unknown deployment mode: ${String(exhaustive)}`] };
      }
    }
...
export function resolveConfigEnvironment(
  ...
  if (config.env) {
    resolved.env = Object.fromEntries(
      Object.entries(config.env).map(([key, value]) => [key, resolveEnvVariables(value, env)]),
    );
  }
