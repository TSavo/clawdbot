...
export class DiscordVoiceChannelConnector {
  ...
  private transcriptionCallbacks = new Set<(transcription: UserTranscription) => void>();
  ...
  private handleUserStartSpeaking(userId: string): void {
    ...
    void this.processUserAudio(userId, participant);
   ...
   */
  private async processUserAudio(userId: string, participant: VoiceParticipant): Promise<void> {
  ...
  private decodeAndQueueAudio(
    ...
    try {
      ...
      // Create audio buffer
      const duration = (monoPcm.length / 2 / DiscordVoiceChannelConnector.SAMPLE_RATE) * 1000;
