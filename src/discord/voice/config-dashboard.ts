...
function buildVoiceDashboard(params: {
  ...
  switch (level) {
    case "user":
      ...
      break;
    case "channel":
      activeMode = context.channelId ? configs.channels[context.channelId] : undefined;
      break;
    case "guild":
  ...
  const modeButtons = modes.map(
    (mode) => new VoiceModeButton(mode, level, userId, cfg, mode === activeMode),
  );

  // Build level selection buttons
  const levels: VoiceConfigLevel[] = ["user", "channel", "guild", "global"];
  const levelButtons = levels.map((lvl) => new LevelSelectButton(lvl, userId, cfg, lvl === level));
    ...
    const fields: Array<{ name: string; value: string }> = [
      ...
      },
      {
        ...
        value: Object.entries(stats.modeDistribution)
          .map(([mode, count]) => `${MODE_EMOJIS[mode as VoiceResponseType]} \`${mode}\`: ${count}`)
          .join("\n"),
      },
      {
        name: "⭐ Most Popular",
...
export function createVoiceDashboardCommands(cfg: Config): Command[] {
  return [new VoiceConfigDashboardCommand(cfg), new VoiceConfigStatsCommand(cfg)];
}
