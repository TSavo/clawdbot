 ...
 */
function isVoiceChannel(_message: Message): boolean {
 ...
 */
async function convertToMP3(audioBuffer: AudioBuffer, bitrate: number): Promise<Buffer> {
  ...
  try {
    ...
    throw new Error(
      `MP3 conversion failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
 ...
 */
async function convertToOGG(audioBuffer: AudioBuffer, bitrate: number): Promise<Buffer> {
  try {
    // Convert AudioBuffer.data (Uint8Array) to Node.js Buffer
    const pcmBuffer = Buffer.from(audioBuffer.data);
    ...
    throw new Error(
      `OGG conversion failed: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
 ...
 */
export async function handleVoiceMessageResponse(context: VoiceResponseContext): Promise<void> {
  ...
  if (!isVoiceMessagesEnabled(context.voiceConfig)) {
    ...
    await sendTextMessage(context.message.channelId, context.responseText, context, {
...
async function sendVoiceResponse(context: VoiceResponseContext): Promise<void> {
  ...
  const audioData =
    context.voiceConfig.voiceFormat === "mp3"
  ...
  // Send to Discord
  await sendVoiceMessage(context.message.channelId, audioData, filename, context, {
    messageReference: context.message.id,
  });
}
 ...
 */
export function isResponseTooLarge(textLength: number, voiceConfig: DiscordVoiceConfig): boolean {
