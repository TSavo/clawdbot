...
export function hasVoiceConfig(
  ...
  switch (level) {
    case "guild":
      return Boolean(context.guildId && voiceConfig.perGuildOverride?.[context.guildId]);

    case "channel":
      return Boolean(context.channelId && voiceConfig.perChannelOverride?.[context.channelId]);
