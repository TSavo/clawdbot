...
import type { VoiceChannelConfig, UserTranscription } from "./channel-connector.js";
...
export class DiscordVoiceRestAPI {
   ...
   */
  async handleJoin(request: JoinVoiceChannelRequest): Promise<JoinVoiceChannelResponse> {
    try {
      // Validate request
      ...
      if (!this.adapterCreatorFactory) {
        return {
          ...
          guildId: request.guildId,
          channelId: request.channelId,
          error: "Discord adapter not initialized. Ensure Discord client is ready.",
        };
      }
   ...
   */
  async handleLeave(request: LeaveVoiceChannelRequest): Promise<LeaveVoiceChannelResponse> {
    try {
      // Validate request
      if (!request.guildId || !request.channelId) {
        return {
   ...
   */
  async handleBroadcast(request: BroadcastAudioRequest): Promise<BroadcastAudioResponse> {
    try {
      // Validate request
      ...
      // Create audio buffer (prefixed to satisfy linter - implementation pending)
   ...
   */
  setupTranscriptionForwarding(callback: (transcription: UserTranscription) => void): void {
   ...
   */
  removeTranscriptionForwarding(callback: (transcription: UserTranscription) => void): void {
