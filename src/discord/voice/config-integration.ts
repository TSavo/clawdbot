...
import { createVoiceConfigCommands } from "./config-commands.js";
import {
...
export async function createVoiceConfigBot(params: {
  ...
  const commands = [...createVoiceConfigCommands(cfg), ...createVoiceDashboardCommands(cfg)];
...
export function addVoiceConfigToMonitor(params: {
  ...
  const allCommands = [...createVoiceConfigCommands(cfg), ...createVoiceDashboardCommands(cfg)];
