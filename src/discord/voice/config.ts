...
export function validateVoiceConfig(config: Partial<DiscordVoiceConfig>): string[] {
  ...
  if (
    config.messageResponse &&
    !["voice", "text", "both", "match"].includes(config.messageResponse)
  ) {
 ...
 */
export function mergeVoiceConfig(config: Partial<DiscordVoiceConfig>): DiscordVoiceConfig {
  return {
