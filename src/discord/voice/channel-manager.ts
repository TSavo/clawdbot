...
export class DiscordVoiceChannelManager {
  ...
  private globalTranscriptionCallbacks = new Set<(transcription: UserTranscription) => void>();
  ...
  async join(config: VoiceChannelConfig): Promise<string> {
    ...
    try {
      // Create connector
      const connector = new DiscordVoiceChannelConnector(config, this.sttProvider);
  ...
  async leave(guildId: string, channelId: string): Promise<void> {
    ...
    if (!managed) {
      throw new Error(`Not connected to channel ${channelId} in guild ${guildId}`);
    }
    ...
    try {
      ...
      console.log(`Manager: Left voice channel ${channelId} in guild ${guildId}`);
    } catch (error) {
      console.error(
   ...
   */
  private handleTranscription(connectionId: string, transcription: UserTranscription): void {
