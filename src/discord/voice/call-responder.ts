...
export class DiscordCallResponder {
   ...
   */
  async generateAndPlayResponse(context: CallResponseContext): Promise<CallResponseResult> {
    ...
    try {
      ...
      const chunks = this.chunkResponse(responseText, context.config.maxChunkLength ?? 500);
   ...
   */
  private async synthesizeChunk(text: string, context: CallResponseContext): Promise<AudioBuffer> {
    if (!this.voiceRegistry) {
