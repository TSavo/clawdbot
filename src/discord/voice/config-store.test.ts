...
describe("config-store", () => {
  ...
  describe("setVoiceResponseType", () => {
    ...
    it("should set guild override", () => {
      ...
      expect(cfg.channels?.discord?.voice?.perGuildOverride?.guild1).toBe("text");
    });
    ...
    it("should set channel override", () => {
      ...
      expect(cfg.channels?.discord?.voice?.perChannelOverride?.channel1).toBe("both");
    });
    ...
    it("should set user override", () => {
      ...
      expect(cfg.channels?.discord?.voice?.perUserOverride?.user1).toBe("match");
    });
    ...
    it("should throw when channel ID missing for channel level", () => {
      ...
      expect(() => setVoiceResponseType(cfg, "channel", "voice", context)).toThrow(
        "Channel ID required",
      );
    });
    ...
    it("should throw when user ID missing for user level", () => {
      ...
      expect(() => setVoiceResponseType(cfg, "user", "voice", context)).toThrow("User ID required");
    });
  });
  ...
  describe("hasVoiceConfig", () => {
    ...
    it("should return false when no voice config exists", () => {
      ...
      expect(hasVoiceConfig(cfg, "channel", { channelId: "channel1" })).toBe(false);
    });
  });
});
