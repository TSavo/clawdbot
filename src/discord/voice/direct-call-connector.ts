...
import type { AudioBuffer, VoiceProviderExecutor } from "../../media/voice-providers/executor.js";
...
export class DiscordDirectCallConnector {
  ...
  private sttProvider: VoiceProviderExecutor;
  ...
  private transcriptionCallbacks = new Set<(transcription: DirectCallTranscription) => void>();

  // Response generation
  private responseCallbacks = new Set<(text: string) => Promise<string>>();

  constructor(config: DirectCallConfig, sttProvider: VoiceProviderExecutor) {
    this.config = config;
  ...
  private handleUserStartSpeaking(): void {
    ...
    // Create opus decoder
    this.opusDecoder = new OpusEncoder(AUDIO_CONSTANTS.SAMPLE_RATE, AUDIO_CONSTANTS.CHANNELS);
  ...
  private async processUserAudio(): Promise<void> {
    ...
    this.userAudioStream.on("error", (error) => {
      logger.error({ error: error.message, userId: this.config.userId }, "Audio stream error");
    });
  }
  ...
  private decodeAndQueueAudio(opusPackets: Buffer[], decoder: OpusEncoder): void {
    try {
      ...
      // Create audio buffer
      const duration = (monoPcm.length / 2 / AUDIO_CONSTANTS.SAMPLE_RATE) * 1000;
  ...
  private async processAudioQueue(): Promise<void> {
    while (this.audioQueue.length > 0) {
      ...
      try {
        ...
        // Stream to Deepgram
        const transcriptionStream = this.sttProvider.transcribeStream(audioStream);
  ...
  private setupConnectionHandlers(): void {
    ...
    this.connection.on("error", (error) => {
      logger.error({ error: error.message, userId: this.config.userId }, "Voice connection error");
    });

    if (this.audioPlayer) {
      this.audioPlayer.on("error", (error) => {
        logger.error({ error: error.message, userId: this.config.userId }, "Audio player error");
      });
