 ...
 */
function hasPermission(interaction: CommandInteraction, level: VoiceConfigLevel): boolean {
...
export class VoiceConfigGetCommand extends Command {
  ...
  async run(interaction: CommandInteraction) {
    ...
    const embed = buildConfigEmbed({
      ...
      description: "Settings are prioritized: User > Channel > Server > Global",
      fields,
    });
...
export class VoiceConfigSetCommand extends Command {
  ...
  async run(interaction: CommandInteraction) {
    ...
    if (!hasPermission(interaction, level)) {
      await interaction.reply({
        content: "❌ You need Administrator permission to change server or channel settings.",
        ephemeral: true,
      });
      return;
    }
...
export class VoiceConfigResetCommand extends Command {
  ...
  async run(interaction: CommandInteraction) {
    ...
    if (!hasPermission(interaction, level)) {
      await interaction.reply({
        content: "❌ You need Administrator permission to reset server or channel settings.",
        ephemeral: true,
      });
      return;
    }
