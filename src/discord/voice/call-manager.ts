...
export class DiscordCallManager {
  ...
  async initiateCall(params: {
    ...
    if (this.activeCalls.has(userId)) {
      ...
      throw new Error(`Call already exists for user ${userId} in state ${existing.state}`);
    }

    // Check concurrent call limit
    const activeCallCount = this.getActiveCalls().length;
    if (activeCallCount >= this.config.maxConcurrentCalls) {
      throw new Error(`Maximum concurrent calls reached (${this.config.maxConcurrentCalls})`);
    }
  ...
  async declineCall(userId: string): Promise<void> {
    ...
    if (record.state !== CallState.RINGING) {
      throw new Error(`Cannot decline call in state ${record.state}`);
    }
   ...
   * Get call history
   */
  getCallHistory(params?: { userId?: string; limit?: number; since?: number }): CallRecord[] {
  ...
  getStats(): {
    ...
    const averageDuration =
      completedCalls.length > 0
        ? completedCalls.reduce((sum, r) => sum + (r.duration || 0), 0) / completedCalls.length
        : 0;
  ...
  private cleanup(): void {
    ...
    for (const [userId, record] of this.activeCalls.entries()) {
      ...
      if (record.state === CallState.FAILED || record.state === CallState.TIMEOUT) {
        this.moveToHistory(userId);
      }
    }
    ...
    this.callHistory = this.callHistory.filter((record) => record.startTime >= cutoffTime);
