...
import { hasAudioAttachment, getAudioAttachments, processAudioMessage } from "./message-handler.js";
...
export async function handleDiscordMessage(params: {
  ...
  try {
    ...
    const { audioData, metadata } = await processAudioMessage(message, attachment, {
      channelName,
      guildName,
      userName,
      runtime,
    ...
    const _transcribedText = "[Audio message transcription would go here]";
...
export async function completeVoiceMessageFlow(params: {
  ...
  try {
    ...
    const audioAttachments = getAudioAttachments(message);
    const { audioData: _audioData, metadata: _metadata } = await processAudioMessage(message, audioAttachments[0], {
      runtime,
    });

    // Step 4: Transcribe (placeholder)
    const _transcription = "[Transcription]";
