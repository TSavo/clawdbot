...
export class CallProviderRegistry {
    providers = new Map();
    platformMap = new Map(); // platform -> providerId
    constructor() { }
    /**
 ...
 * ```
 */
export async function initializeAllCallProviders(config) {
    ...
    if (enabledPlatforms.includes("discord") && config.discordToken) {
        try {
            const { DiscordCallProviderPlugin } = await import("./discord/provider.js");
            ...
            console.warn(`Failed to initialize Discord call provider: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // Initialize Telegram call provider
    if (enabledPlatforms.includes("telegram") && config.telegramToken) {
        try {
            const { TelegramCallProviderPlugin } = await import("./telegram/provider.js");
            ...
            console.warn(`Failed to initialize Telegram call provider: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // Initialize Signal call provider
    if (enabledPlatforms.includes("signal") && config.signalCliUrl) {
        try {
            const { SignalCallProviderPlugin } = await import("./signal/provider.js");
            ...
            console.warn(`Failed to initialize Signal call provider: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    // Initialize Twilio call provider
    if (enabledPlatforms.includes("twilio") && config.twilioConfig) {
        try {
            const { TwilioCallProviderPlugin } = await import("./twilio/twilio-provider.js");
            const { TwilioProvider } = await import("../providers/twilio.js");
            const twilioProvider = new TwilioProvider(config.twilioConfig.accountSid, config.twilioConfig.authToken);
            ...
            console.warn(`Failed to initialize Twilio call provider: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return registry;
}
 ...
 */
export function getProviderMetadata(registry) {
