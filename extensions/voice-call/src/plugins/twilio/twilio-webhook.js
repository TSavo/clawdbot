...
import { isDtmfEvent, isCallStateEvent, isSpeechEvent, TWILIO_STATUS_MAP, } from "./twilio-config.js";
// ============================================================================
 ...
 */
function reconstructUrlForVerification(ctx, currentPublicUrl) {
    ...
    const forwardedProto = ctx.headers["x-forwarded-proto"] || ctx.headers["x-proto"] || "https";
    const forwardedHost = ctx.headers["x-forwarded-host"] || ctx.headers["host"] || "localhost";
 ...
 */
export function parseTwilioWebhookPayload(params, callIdOverride) {
    ...
    if (isCallStateEvent(params)) {
        ...
        if (callStatus === "initiated" ||
            ...
            return {
                ...baseEvent,
                type: `call.${callStatus === "in-progress" ? "answered" : callStatus}`,
            };
        }
 ...
 */
export function generateMinimalTwimlResponse() {
 ...
 */
export function generatePauseTwimlResponse(seconds = 30) {
    return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
 ...
 */
export function generateStreamingTwimlResponse(streamUrl) {
    return `<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Connect>
 ...
 */
export function extractRecordingMetadata(params) {
    return {
        recordingUrl: params.get("RecordingUrl") || undefined,
        recordingStatus: params.get("RecordingStatus") || undefined,
        recordingDuration: parseInt(params.get("RecordingDuration") || "0", 10) || undefined,
        recordingChannels: parseInt(params.get("RecordingChannels") || "1", 10) || undefined,
    };
}
// ============================================================================
 ...
 */
export function parseMediaStreamConnected(params) {
 ...
 */
export function parseMediaStreamEvent(params) {
 ...
 */
export function validateTwilioWebhookPayload(params) {
    const callSid = params.get("CallSid");
    if (!callSid) {
