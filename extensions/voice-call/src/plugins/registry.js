 ...
 */
import { PluginRegistryError, } from "./interfaces.js";
/**
...
export class PluginRegistry {
     ...
     */
    registerSTT(id, provider, config) {
        try {
     ...
     * @returns Registration result
     */
    registerTTS(id, provider, config) {
        try {
            provider.validateConfig();
            const registration = {
                id,
    ...
    getSTT(id) {
        ...
        if (!registration) {
            throw new PluginRegistryError(`Plugin '${id}' not found`, "PLUGIN_NOT_FOUND");
        }
        if (registration.type !== "stt") {
            throw new PluginRegistryError(`Plugin '${id}' is not an STT provider`, "PLUGIN_TYPE_MISMATCH");
        }
    ...
    getTTS(id) {
        ...
        if (registration.type !== "tts") {
            throw new PluginRegistryError(`Plugin '${id}' is not a TTS provider`, "PLUGIN_TYPE_MISMATCH");
        }
        return registration.instance;
    }
    /**
