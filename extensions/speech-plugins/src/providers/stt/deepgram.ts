...
export class DeepgramSTTPlugin extends BaseSTTPlugin {
    ...
    this.metadata = {
      ...
      configSchema: {
        validate: (config: unknown) => {
          ...
          if (cfg.model && !['nova-v3', 'flux'].includes(cfg.model as string)) {
            return {
              ok: false,
   ...
   */
  private buildDeepgramConfig(): Record<string, unknown> {
    ...
    if (this.deploymentConfig.mode === 'cloud') {
      const cloudConfig = this.deploymentConfig as any;
   ...
   */
  private getDeepgramCapabilities(): STTCapabilities {
    return {
