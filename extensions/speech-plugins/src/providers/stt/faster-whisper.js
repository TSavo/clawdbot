        ...
        this.metadata = {
            ...
            configSchema: {
                validate: (config) => {
                    ...
                    if (cfg.computeType && !['int8', 'float16', 'float32'].includes(cfg.computeType)) {
                        return {
                            ok: false,
    ...
    buildFasterWhisperConfig() {
        ...
        if (this.deploymentConfig.mode === 'system') {
            ...
            if (systemConfig.cliFlags?.defaults) {
                ...
                if (flags.computeType)
                    config.computeType = flags.computeType;
                if (flags.cpuThreads)
                    config.cpuThreads = flags.cpuThreads;
                if (flags.beamSize)
                    config.beamSize = flags.beamSize;
                if (flags.device)
...
//# sourceMappingURL=faster-whisper.js.map