 ...
 */
export function generateSineWave(durationMs, frequency = 440, sampleRate = 16000, amplitude = 0.3) {
 ...
 */
export function generateWhiteNoise(durationMs, sampleRate = 16000, amplitude = 0.3) {
    const numSamples = (durationMs * sampleRate) / 1000;
    const audioData = Buffer.alloc(numSamples * 2);
    for (let i = 0; i < numSamples; i++) {
 ...
 */
export function generateSpeechPattern(durationMs, sampleRate = 16000) {
    const numSamples = (durationMs * sampleRate) / 1000;
    const audioData = Buffer.alloc(numSamples * 2);
 ...
 */
export function createWAVBuffer(audioData, sampleRate = 16000, channels = 1, bitsPerSample = 16) {
...
export const AudioBufferUtils = {
     ...
     */
    concat: (buffers, sampleRate = 16000) => {
     ...
     */
    getDurationMs: (buffer, sampleRate = 16000) => {
     ...
     */
    silence: (durationMs, sampleRate = 16000) => {
        const numSamples = (durationMs * sampleRate) / 1000;
     ...
     * Fade in audio
     */
    fadeIn: (buffer, durationMs = 500, sampleRate = 16000) => {
     ...
     * Fade out audio
     */
    fadeOut: (buffer, durationMs = 500, sampleRate = 16000) => {
        const fadeSamples = (durationMs * sampleRate) / 1000;
        const faded = Buffer.from(buffer);
...
//# sourceMappingURL=audio-fixtures.js.map